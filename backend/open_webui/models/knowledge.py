...
class KnowledgeTable:
    def _extract_linked_knowledge_ids(self, meta: Optional[dict]) -> list[str]:
        if not isinstance(meta, dict):
            return []

        candidates = []
        direct_id = meta.get("knowledge_id")
        if isinstance(direct_id, str) and direct_id:
            candidates.append(direct_id)

        nested_meta = meta.get("data")
        if isinstance(nested_meta, dict):
            nested_id = nested_meta.get("knowledge_id")
    ...
    def _backfill_metadata_links(
        self, knowledge_id: Optional[str] = None, db: Optional[Session] = None
    ) -> int:
        """Recover missing knowledge_file rows from legacy file metadata hints.

        Earlier Dochat iterations stored the target collection only in file meta
        during upload. If vector processing failed before the relation row was
        inserted, the UI would show the file transiently and lose it on reload.
        This keeps the canonical knowledge_file table in sync whenever we read
        collection data.
        """
        created = 0
        ...
        with get_db_context(db) as db:
            try:
                knowledge_ids = None
                if knowledge_id:
                    if not db.query(Knowledge).filter_by(id=knowledge_id).first():
                        return 0
                    knowledge_ids = {knowledge_id}

                files = db.query(File).filter(File.meta.isnot(None)).all()
                for file in files:
                    linked_ids = self._extract_linked_knowledge_ids(file.meta or {})
                    if knowledge_ids is not None:
                        linked_ids = [
                            linked_id
                            for linked_id in linked_ids
                            if linked_id in knowledge_ids
                        ]
                    ...
                    for linked_id in linked_ids:
                        if not db.query(Knowledge).filter_by(id=linked_id).first():
                            continue

                        exists = (
                            db.query(KnowledgeFile)
                            .filter_by(knowledge_id=linked_id, file_id=file.id)
                            .first()
                        )
                        if exists:
                            continue
    ...
    def search_knowledge_files(
        ...
        """
        try:
            self._backfill_metadata_links(db=db)
            with get_db_context(db) as db:
    ...
    def search_files_by_id(
        ...
        try:
            self._backfill_metadata_links(knowledge_id=knowledge_id, db=db)
            with get_db_context(db) as db:
                query = (
                    db.query(File, User)
                    .join(KnowledgeFile, File.id == KnowledgeFile.file_id)
