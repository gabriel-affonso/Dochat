...
class S3StorageProvider(StorageProvider):
    def __init__(self):
        if boto3 is None or Config is None:
            raise RuntimeError(
                "S3 storage requires optional dependencies. Install boto3/botocore or use STORAGE_PROVIDER=local."
            )
...
class GCSStorageProvider(StorageProvider):
    def __init__(self):
        if storage is None:
            raise RuntimeError(
                "GCS storage requires optional dependencies. Install google-cloud-storage or use STORAGE_PROVIDER=local."
            )
...
class AzureStorageProvider(StorageProvider):
    def __init__(self):
        if BlobServiceClient is None or DefaultAzureCredential is None:
            raise RuntimeError(
                "Azure storage requires optional dependencies. Install azure-storage-blob and azure-identity or use STORAGE_PROVIDER=local."
            )
