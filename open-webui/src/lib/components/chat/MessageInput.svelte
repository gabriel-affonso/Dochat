			...
			>
				<div class="relative">
					{#if autoScroll === false && history?.currentId}
						...
						>
							<button
								class="dochat-floating-action pointer-events-auto"
								on:click={() => {
									autoScroll = true;
									scrollToBottom();
								}}
		...
		<div class="bg-transparent">
			<div
				class="{($settings?.widescreenMode ?? null)
					? 'max-w-full'
					: 'max-w-[78rem]'} px-2.5 mx-auto inset-x-0"
			>
				<div class="">
					<input
					...
					>
						...
						{#if messageQueue.length > 0}
							<div
								class="dochat-queue-panel mb-1 mx-2 py-1 px-1.5 overflow-x-hidden overflow-y-auto max-h-[25vh]"
							>
						...
						<div
							...
							class="dochat-input-shell flex-1 flex flex-col relative w-full border {$temporaryChatEnabled
								? 'border-dashed'
								: ''} transition px-1"
						...
						>
							{#if atSelectedModel !== undefined}
								<div class="px-3 pt-3 text-left w-full flex flex-col z-10">
									<div class="flex items-center justify-between w-full">
										<div class="pl-[1px] flex items-center gap-2 text-sm text-[var(--dochat-text-soft)]">
											<img
												alt="model profile"
										...
										<div>
											<button
												class="flex items-center text-[var(--dochat-text-soft)]"
												on:click={() => {
													atSelectedModel = undefined;
												}}
												aria-label={$i18n.t('Clear selected model')}
											>
												<XMark />
											</button>
										</div>
							...
							<div class="px-2.5">
								<div
									class="dochat-input-editor scrollbar-hidden rtl:text-right ltr:text-left bg-transparent outline-hidden w-full pb-1 px-1 resize-none h-fit max-h-96 overflow-auto {files.length ===
									0
										? atSelectedModel !== undefined
											? 'pt-1.5'
											: 'pt-2.5'
									...
									{#if prompt.split('\n').length > 2}
										<div class="fixed top-0 right-0 z-20">
											<div class="mt-2.5 mr-3">
												<button
													type="button"
													class="dochat-toolbar-button p-1 rounded-lg"
										...
										{#key $settings?.richTextInput ?? true}
											{#key $settings?.showFormattingToolbar ?? false}
												<RichTextInput
													...
													placeholder={placeholder ? placeholder : 'No que vamos pensar hoje?'}
							...
							<div class="dochat-input-toolbar flex justify-between mt-1 mb-2.5 mx-0.5 max-w-full" dir="ltr">
								<div class="ml-1 self-end flex items-center flex-1 max-w-[80%]">
									...
									>
										<div
											...
											class="dochat-toolbar-button size-8 flex justify-center items-center outline-hidden focus:outline-hidden"
											aria-label={$i18n.t('Add attachment or context')}
										>
										...
										{#if showWebSearchButton || showImageGenerationButton || showCodeInterpreterButton || showToolsButton || (toggleFilters && toggleFilters.length > 0)}
											<div
												class="flex self-center w-[1px] h-4 mx-1 bg-[rgba(221,214,202,0.9)]"
											></div>
										...
										>
											<div
												...
												class="dochat-toolbar-button size-8 flex justify-center items-center outline-hidden focus:outline-hidden"
												aria-label={$i18n.t('Open tools and integrations')}
											>
									...
									<div class="ml-1 flex gap-1.5">
										{#if (selectedToolIds ?? []).length > 0}
											...
											>
												<button
													class="translate-y-[0.5px] px-1 flex gap-1 items-center text-[var(--dochat-text-soft)] hover:text-[var(--dochat-text)] rounded-lg self-center transition"
											...
											{#if filter}
												<Tooltip content={filter?.name} placement="top">
													<button
														...
														type="button"
														aria-label={$i18n.t('Remove {{name}} filter', { name: filter?.name ?? filterId })}
									...
									{#if (taskIds && taskIds.length > 0) || (history.currentId && history.messages[history.currentId]?.done != true) || generating}
										<div class=" flex items-center">
											<Tooltip content={$i18n.t('Stop')}>
												<button
													class="dochat-primary-action transition rounded-full p-1.5"
													on:click={() => {
														stopResponse();
													}}
										...
										{#if prompt !== '' && !history?.currentId && ($config?.features?.enable_notes ?? false) && ($_user?.role === 'admin' || ($_user?.permissions?.features?.notes ?? true))}
											...
											<Tooltip content={$i18n.t('Create note')} className=" flex items-center">
												<button
													...
													class="text-[var(--dochat-text-soft)] hover:text-[var(--dochat-text)] transition rounded-full p-1.5 self-center"
													type="button"
													disabled={prompt === '' && files.length === 0}
													on:click={() => {
														createNote();
													}}
													aria-label={$i18n.t('Create note from prompt')}
												>
											...
											{#if $_user?.role === 'admin' || ($_user?.permissions?.chat?.stt ?? true)}
												...
												<Tooltip content={$i18n.t('Dictate')}>
													<button
														...
														class="text-[var(--dochat-text-soft)] hover:text-[var(--dochat-text)] transition rounded-full p-1.5 self-center mr-0.5"
														type="button"
														on:click={async () => {
															try {
										...
										{#if prompt === '' && files.length === 0 && ($_user?.role === 'admin' || ($_user?.permissions?.chat?.call ?? true))}
											<div class=" flex items-center">
												...
												<Tooltip content={$i18n.t('Voice mode')}>
													<button
														class="dochat-primary-action transition rounded-full p-1.5 self-center"
														type="button"
														on:click={async () => {
												...
												>
													<button
														...
														class="{!(prompt === '' && files.length === 0) || uploadPending
															? 'dochat-primary-action'
															: 'dochat-primary-action-disabled'} transition rounded-full p-1.5 self-center"
														type="submit"
...
<style>
	...
	.dochat-queue-panel {
		...
		box-shadow: 0 10px 24px rgba(84, 74, 58, 0.05);
	}
	...
	.dochat-input-shell {
		...
		background:
			linear-gradient(180deg, rgba(255, 255, 255, 0.94), rgba(251, 249, 245, 0.94));
		color: var(--dochat-text, #1d1d1f);
		box-shadow: 0 22px 48px rgba(84, 74, 58, 0.08);
		backdrop-filter: blur(18px);
	}

	.dochat-input-shell:focus-within {
		border-color: rgba(111, 138, 100, 0.28);
		box-shadow:
			0 24px 50px rgba(84, 74, 58, 0.09),
			0 0 0 4px rgba(232, 239, 228, 0.6);
	}
	...
	.dochat-input-editor :global(.ProseMirror) {
		min-height: 4.5rem;
	...
	.dochat-input-editor :global(.ProseMirror p.is-editor-empty:first-child::before) {
		color: var(--dochat-text-faint, #8e8e93);
	}

	.dochat-input-toolbar {
		color: var(--dochat-text-soft, #5c5c62);
	}

	.dochat-toolbar-button {
		border-radius: 9999px;
		color: var(--dochat-text-soft, #5c5c62);
	...
	.dochat-primary-action:hover {
		background: var(--dochat-accent-hover, #647c5a);
	}

	.dochat-primary-action-disabled {
		background: rgba(221, 214, 202, 0.96);
		color: rgba(92, 92, 98, 0.82);
	}
</style>
