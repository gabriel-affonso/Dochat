<script lang="ts">
	...
	const getVisibleAvailableModels = () =>
		$models.filter((model) => !(model?.info?.meta?.hidden ?? false)).map((model) => model.id);

	const resolvePreferredSelectedModels = () => {
		const availableModels = getVisibleAvailableModels();
		...
		const configuredModels = [
			...
			...($config?.default_models
				? $config.default_models.split(',').map((modelId) => modelId.trim())
				: [])
		].filter(Boolean);

		const preferredModels = configuredModels.filter((modelId) => availableModels.includes(modelId));

		return preferredModels.length > 0 ? preferredModels : [availableModels[0]];
	};

	const ensureSelectedModelsAvailable = () => {
		if (atSelectedModel !== undefined) {
			return;
		}
		...
		const currentSelection = selectedModels.filter(Boolean);
		const validSelection = currentSelection.filter((modelId) => availableModels.includes(modelId));
		const nextSelection =
			validSelection.length > 0 ? validSelection : resolvePreferredSelectedModels();

		if (
			nextSelection.length > 0 &&
			JSON.stringify(nextSelection) !== JSON.stringify(selectedModels)
		) {
			selectedModels = nextSelection;
		}
	};

	$: if ($models.length > 0) {
		ensureSelectedModelsAvailable();
	}
	...
	$: activeMessageCount = history?.currentId
		? createMessagesList(history, history.currentId).length
		: 0;
	$: hasConversation =
		($settings?.landingPageMode === 'chat' && !$selectedFolder) || activeMessageCount > 0;
	$: documentContextCount = chatFiles.filter(
		(item) =>
			['doc', 'text', 'note', 'chat', 'folder', 'collection'].includes(item?.type) ||
			(item?.type === 'file' && !(item?.content_type ?? '').startsWith('image/'))
	).length;
	$: documentChatMode = documentContextCount > 0;
	$: backgroundImageUrl =
	...
	const chatCompletionEventHandler = async (data, message, chatId) => {
		...
		if (retrieval_audit) {
			message.retrieval_audit = retrieval_audit;
		}
		...
		try {
			...
			if (res && res.ok && res.body && generating) {
				...
				for await (const update of textStream) {
					const { value, done, sources, retrievalAudit, error, usage } = update;
					if (error || done) {
						generating = false;
						generationController = null;
						break;
					...
					if (retrievalAudit) {
						mergedResponse.retrieval_audit = retrievalAudit;
						history.messages[messageId] = message;
						continue;
					}
...
<div
	class="dochat-chat-shell h-screen max-h-[100dvh] transition-width duration-200 ease-in-out {$showSidebar
		? '  md:max-w-[calc(100%-var(--sidebar-width))]'
		: ' '} w-full max-w-full flex flex-col"
	id="chat-container"
>
	{#if !loading}
		<div in:fade={{ duration: 50 }} class="dochat-chat-frame w-full h-full flex flex-col">
			<div class="dochat-chat-background" aria-hidden="true"></div>
			{#if backgroundImageUrl}
				<div
					class="dochat-chat-image absolute inset-0 bg-cover bg-center bg-no-repeat"
					style="background-image: url({backgroundImageUrl})"
				></div>
			{/if}
			<div class="dochat-chat-overlay" aria-hidden="true"></div>

			<PaneGroup direction="horizontal" class="w-full h-full relative">
				<Pane
					defaultSize={50}
					minSize={30}
					class="h-full flex relative max-w-full flex-col px-2 md:px-4 pt-3 pb-2 max-md:px-2 max-md:pt-2.5 max-md:pb-2"
				>
					...
					<div
						id="chat-pane"
						class="dochat-chat-pane flex flex-col flex-auto z-10 w-full @container overflow-auto"
					>
						<div class="dochat-chat-pane-shell">
							{#if documentChatMode}
								<div class="dochat-chat-context-banner">
									<div class="dochat-chat-context-copy">
										<span class="dochat-chat-context-kicker">Chat com documento</span>
										<div class="dochat-chat-context-title">
											{documentContextCount}
											{documentContextCount === 1 ? 'fonte ativa' : 'fontes ativas'}
										</div>
									</div>

									<button
										type="button"
										class="dochat-chat-context-action"
										on:click={() => {
											showControls.set(true);
										}}
									>
							...
							{#if hasConversation}
								<div
									class="dochat-messages-panel pb-2.5 flex flex-col justify-between w-full flex-auto overflow-auto h-0 max-w-full z-10 scrollbar-hidden"
								...
								<div class="dochat-composer-slot pb-2 z-10">
									<MessageInput
									...
									<div
										class="absolute bottom-1 text-xs text-[var(--dochat-text-faint)] text-center line-clamp-1 right-0 left-0"
									>
										<!-- {$i18n.t('LLMs can make mistakes. Verify important information.')} -->
									</div>
								</div>
							{:else}
								<div class="dochat-placeholder-shell flex items-center h-full">
									<Placeholder
										{history}
										{selectedModels}
										bind:messageInput
...
<style>
	...
	.dochat-chat-shell {
		position: relative;
		background:
			radial-gradient(circle at top left, rgba(232, 239, 228, 0.7), transparent 34%),
			linear-gradient(180deg, rgba(251, 249, 245, 0.96), rgba(247, 244, 238, 0.98));
	...
	.dochat-chat-background {
		...
		inset: 0;
		background:
			radial-gradient(circle at top, rgba(255, 255, 255, 0.86), transparent 42%),
			radial-gradient(circle at bottom right, rgba(232, 239, 228, 0.6), transparent 34%);
		z-index: -3;
	}
	...
	.dochat-chat-overlay {
		...
		inset: 0;
		background:
			linear-gradient(180deg, rgba(251, 249, 245, 0.94), rgba(247, 244, 238, 0.88)),
			linear-gradient(90deg, rgba(251, 249, 245, 0.72), rgba(251, 249, 245, 0.3));
	...
	.dochat-chat-context-title {
		font-size: 0.98rem;
		font-weight: 600;
		color: var(--dochat-text, #1d1d1f);
	}

	.dochat-chat-context-action {
		border: 1px solid rgba(111, 138, 100, 0.16);
	...
	.dochat-chat-context-action:hover {
		background: rgba(232, 239, 228, 0.92);
		border-color: rgba(111, 138, 100, 0.28);
		transform: translateY(-1px);
	}

	.dochat-messages-panel {
		padding: 0 0 0.25rem;
	}

	.dochat-composer-slot {
		position: relative;
	}

	.dochat-placeholder-shell {
		padding: 0.5rem 0;
	}
