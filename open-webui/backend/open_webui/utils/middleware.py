...
from open_webui.utils.retrieval_audit import (
    build_grounding_guidance,
...
def apply_source_context_to_messages(
    ...
    grounding_guidance = build_grounding_guidance(retrieval_audit)
    rag_content = rag_template(request.app.state.config.RAG_TEMPLATE, context, user_message)
    rag_content = "\n\n".join(
        item for item in [grounding_guidance, rag_content] if item
    )
...
def build_retrieval_message_metadata(metadata: dict) -> dict:
    return {
        **({"sources": metadata.get("sources")} if metadata.get("sources") else {}),
        **(
            {"retrieval_audit": metadata.get("retrieval_audit")}
            if metadata.get("retrieval_audit")
            else {}
        ),
    }
...
async def chat_completion_files_handler(
    ...
    if files := body.get("metadata", {}).get("files", None):
        ...
        if sources_count == 0:
            ...
            log.warning(
                "chat_completion_files_handler:no_sources "
                + f"queries={queries} files={file_summaries} "
                + f"source_wrappers={len(sources)} "
                + f"hybrid={request.app.state.config.ENABLE_RAG_HYBRID_SEARCH} "
                + f"full_context={all_full_context or request.app.state.config.RAG_FULL_CONTEXT}"
            )
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    metadata["retrieval_audit"] = build_retrieval_audit(
        sources,
        assistant_message_id=metadata.get("message_id"),
        user_message_id=metadata.get("parent_message_id"),
        chat_id=metadata.get("chat_id"),
        query_text=prompt,
        model_id=form_data.get("model"),
        top_k_requested=request.app.state.config.TOP_K,
    )
    ...
    if len(sources) > 0:
        event_payload = {"sources": sources}
        if metadata.get("retrieval_audit"):
            event_payload["retrieval_audit"] = metadata.get("retrieval_audit")
                ...
                if content:
                    ...
                    Chats.upsert_message_to_chat_by_id_and_message_id(
                        ...
                        metadata["message_id"],
                        {
                            ...
                            **build_retrieval_message_metadata(metadata),
                        },
                    )
                                        ...
                                        if ENABLE_REALTIME_CHAT_SAVE:
                                            ...
                                            Chats.upsert_message_to_chat_by_id_and_message_id(
                                                ...
                                                metadata["message_id"],
                                                {
                                                    "content": serialize_output(output),
                                                    "output": output,
                                                    **build_retrieval_message_metadata(
                                                        metadata
                                                    ),
                                                },
                                            )
