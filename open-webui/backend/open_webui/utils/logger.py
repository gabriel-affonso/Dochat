...
def _dochat_logger_name(name: str) -> str:
    if name.startswith("open_webui"):
        return name.replace("open_webui", "dochat", 1)
    return name
...
def stdout_format(record: "Record") -> str:
    ...
    return (
        ...
        "<level>{level: <8}</level> | "
        f"<cyan>{_dochat_logger_name(record['name'])}</cyan>:"
...
def _json_sink(message: "Message") -> None:
    ...
    log_entry = {
        ...
        "msg": record["message"],
        "caller": (
            f"{_dochat_logger_name(record['name'])}:"
            f"{record['function']}:{record['line']}"
        ),
    }
