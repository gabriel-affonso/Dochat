...
def get_license_data(app, key):
    ...
    if key:
        us = [
            url.strip()
            for url in os.environ.get("DOCHAT_LICENSE_API_URLS", "").split(",")
            if url.strip()
        ]
        if not us:
            log.info(
                "Remote license lookup is disabled in this standalone Dochat build. "
                "Set DOCHAT_LICENSE_API_URLS to use your own license API endpoints."
            )
        try:
            for u in us:
                if handler(u):
