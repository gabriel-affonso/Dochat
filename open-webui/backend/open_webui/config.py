...
if CUSTOM_NAME:
    log.info(
        "CUSTOM_NAME is set, but remote upstream branding lookup is disabled in "
        "this standalone Dochat build. Local Dochat assets and WEBUI_NAME will be used."
    )
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (Dochat + Ollama)
