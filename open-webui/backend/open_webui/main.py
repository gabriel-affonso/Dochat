...
log = logging.getLogger(__name__.replace("open_webui", "dochat", 1))
...
v{VERSION} - informação em diálogo, memória e decisão.
{f"Commit: {WEBUI_BUILD_HASH}" if WEBUI_BUILD_HASH != "dev-build" else ""}
https://dochat.epilogos.eu
""")
...
async def generate_messages(
    ...
    All models configured in Dochat are accessible via this endpoint.
...
async def get_app_latest_release_version(user=Depends(get_verified_user)):
    ...
    log.debug(
        "Standalone Dochat build does not query upstream GitHub releases; "
        "returning the current version as the latest known version."
    )
    return {"current": VERSION, "latest": VERSION}
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for Dochat.
