...
class FilesTable:
    ...
    def update_file_by_id(
        ...
        with get_db_context(db) as db:
            try:
                ...
                if form_data.archived_at is not None or form_data.is_archived is False:
                    file.archived_at = form_data.archived_at
    ...
    def set_file_archived_state_by_id(
        self,
        id: str,
        is_archived: bool,
        db: Optional[Session] = None,
        ...
        with get_db_context(db) as db:
            try:
                ...
                file.archived_at = int(time.time()) if is_archived else None
                file.updated_at = int(time.time())
                db.commit()
                db.refresh(file)
                return FileModel.model_validate(file)
    ...
    def toggle_file_archive_by_id(
        self, id: str, db: Optional[Session] = None
    ) -> Optional[FileModel]:
        file = self.get_file_by_id(id, db=db)
        if not file:
            return None
        return self.set_file_archived_state_by_id(id, not file.is_archived, db=db)

    def get_archived_files_by_user_id(
        ...
        with get_db_context(db) as db:
            return [
                ...
                .order_by(File.archived_at.desc(), File.updated_at.desc(), File.id.asc())
                .all()
            ]
