...
class ChatMessageTable:
    ...
    def get_latest_message_by_chat_id(
        self,
        chat_id: str,
        ...
        with get_db_context(db) as db:
            query = db.query(ChatMessage).filter_by(chat_id=chat_id)
            if role:
                query = query.filter_by(role=role)
            message = query.order_by(
                ChatMessage.created_at.desc(), ChatMessage.updated_at.desc()
            ).first()
            return ChatMessageModel.model_validate(message) if message else None

    def get_latest_sources_by_chat_id(
        self, chat_id: str, db: Optional[Session] = None
    ) -> list[dict]:
        message = self.get_latest_message_by_chat_id(chat_id, role="assistant", db=db)
