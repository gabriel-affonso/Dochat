...
class ChatTable:
    ...
    def upsert_message_to_chat_by_id_and_message_id(
        self, id: str, message_id: str, message: dict, db: Optional[Session] = None
    ) -> Optional[ChatModel]:
        with get_db_context(db) as db:
    ...
    def unarchive_all_chats_by_user_id(
        ...
        try:
            with get_db_context(db) as db:
                db.query(Chat).filter_by(user_id=user_id).update(
    ...
    def toggle_chat_archive_by_id(
        ...
        try:
            with get_db_context(db) as db:
                ...
                chat.archived_at = int(time.time()) if chat.archived else None
                chat.folder_id = None
                chat.updated_at = int(time.time())
                db.commit()
                db.refresh(chat)
    ...
    def set_chat_archived_state_by_id(
        ...
        try:
            with get_db_context(db) as db:
                ...
                if not chat:
                    return None
                chat.archived = is_archived
                chat.archived_at = int(time.time()) if is_archived else None
                if is_archived:
                    chat.folder_id = None
                chat.updated_at = int(time.time())
                db.commit()
