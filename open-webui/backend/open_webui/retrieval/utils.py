...
def _query_result_document_count(query_result: Optional[dict]) -> int:
    ...
    first_batch = documents[0]
    if not isinstance(first_batch, list):
        return 0

    return sum(1 for document in first_batch if document)
...
async def query_collection_with_fallback(
    collection_names: list[str],
    queries: list[str],
    embedding_function,
    k: int,
    ...
    if hybrid_search:
        try:
            ...
            hybrid_count = _query_result_document_count(hybrid_result)
            if hybrid_count > 0:
                log.info(
                    "query_collection_with_fallback:hybrid_success "
                    + f"collections={sorted_collection_names} queries={queries} count={hybrid_count}"
                )
                return hybrid_result

            log.warning(
                "query_collection_with_fallback:hybrid_empty "
                + f"collections={sorted_collection_names} queries={queries}. "
                + "Falling back to non-hybrid search."
            )
        except Exception:
            log.exception(
                "query_collection_with_fallback:hybrid_failed "
                + f"collections={sorted_collection_names} queries={queries}. "
                + "Falling back to non-hybrid search."
            )
    ...
    query_count = _query_result_document_count(query_result)

    if query_count > 0:
        log.info(
            "query_collection_with_fallback:non_hybrid_success "
            + f"collections={sorted_collection_names} queries={queries} count={query_count} "
            + f"hybrid_attempted={hybrid_search}"
        )
    else:
        log.warning(
            "query_collection_with_fallback:no_results "
            + f"collections={sorted_collection_names} queries={queries} "
            + f"hybrid_attempted={hybrid_search}"
        )
    ...
    for item in items:
        ...
        if item.get("type") == "text":
            ...
            if (
                ...
                if item.get("legacy"):
                    resolved_collection_names.append(f"{item['id']}")
                elif file_id:
                    file_collection_name = f"file-{file_id}"
                    if VECTOR_DB_CLIENT.has_collection(file_collection_name):
                        resolved_collection_names.append(file_collection_name)

                    # Document chat can target files that were indexed only inside
                    # their parent knowledge collections. Reuse those collection
                    # ids so retrieval still works without a standalone file-* index.
                    knowledge_collections = item.get("collections") or []
                    if not knowledge_collections:
                        knowledge_collections = Knowledges.get_knowledges_by_file_id(file_id)

                    for knowledge in knowledge_collections or []:
                        collection_id = (
                            knowledge.get("id")
                            if isinstance(knowledge, dict)
                            else getattr(knowledge, "id", None)
                        )
                        if collection_id and VECTOR_DB_CLIENT.has_collection(collection_id):
                            resolved_collection_names.append(collection_id)

                collection_names.extend(resolved_collection_names)

                if (
                    query_result is None
                    and not collection_names
                    and file_object
                    and file_object.data.get("content", "")
                ):
                    query_result = {
                        "documents": [[file_object.data.get("content", "")]],
                        "metadatas": [
        ...
        if query_result is None and collection_names:
            ...
            try:
                if full_context:
                    ...
                    full_context_count = _query_result_document_count(query_result)
                    if full_context_count == 0:
                        log.warning(
                            "get_sources_from_items:full_context_empty "
                            + f"collections={sorted(collection_names)}"
                        )
                else:
                    query_result = await query_collection_with_fallback(
                        collection_names=collection_names,
                        queries=queries,
                        embedding_function=embedding_function,
                        k=k,
