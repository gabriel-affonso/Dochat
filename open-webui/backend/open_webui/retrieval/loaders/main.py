...
from open_webui.env import DOCLING_SERVER_TIMEOUT, GLOBAL_LOG_LEVEL, REQUESTS_VERIFY
    ...
    def load(self) -> list[Document]:
        with open(self.file_path, "rb") as f:
            ...
            r = requests.post(
                ...
                headers=headers,
                timeout=DOCLING_SERVER_TIMEOUT,
            )
        if r.ok:
            result = r.json()
        ...
        if (
            ...
            if self.kwargs.get("DOCUMENT_INTELLIGENCE_KEY") != "":
                ...
                if DefaultAzureCredential is None:
                    raise RuntimeError(
                        "Azure Document Intelligence with managed identity requires azure-identity. Install it or configure DOCUMENT_INTELLIGENCE_KEY."
                    )
                loader = AzureAIDocumentIntelligenceLoader(
                    file_path=file_path,
                    api_endpoint=self.kwargs.get("DOCUMENT_INTELLIGENCE_ENDPOINT"),
