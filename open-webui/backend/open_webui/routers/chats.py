...
from open_webui.utils.dochat_documents import build_document_payload
...
class ChatDocumentAttachForm(BaseModel):
    document_id: str
    message_id: Optional[str] = None
    set_active: bool = True


class ChatDocumentContextResponse(BaseModel):
    chat_id: str
...
def _get_chat_for_user(id: str, user, db: Session):
    chat = (
        Chats.get_chat_by_id(id, db=db)
        if user.role == "admin"
        else Chats.get_chat_by_id_and_user_id(id, user.id, db=db)
    )
    if not chat:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
def _build_chat_documents(chat_id: str, user, db: Session) -> list[dict]:
    documents = []
    seen_file_ids = set()
    for chat_file in Chats.get_chat_files_by_chat_id(chat_id, db=db):
        if chat_file.file_id in seen_file_ids:
            continue
        file = Files.get_file_by_id(chat_file.file_id, db=db)
        if not file:
            continue
        if not (
            user.role == "admin"
            or file.user_id == user.id
            or has_access_to_file(file.id, "read", user, db=db)
        ):
            continue
        seen_file_ids.add(chat_file.file_id)
        documents.append(
            build_document_payload(
                file,
                Knowledges.get_knowledges_by_file_id(file.id, db=db),
            )
        )
    return documents


def _sync_chat_document_context(
    chat,
    user,
    db: Session,
    ...
    documents = _build_chat_documents(chat.id, user, db)
    current_context = (chat.meta or {}).get("document_context") or {}
    ...
    if active_document_id and not any(
        document["id"] == active_document_id for document in documents
    ):
        active_document_id = None

    resolved_active_document_id = (
        active_document_id
        or current_context.get("active_document_id")
        or (documents[0]["id"] if documents else None)
    )
    active_document = next(
        (
            document
            for document in documents
            if document["id"] == resolved_active_document_id
        ),
        None,
    )
    resolved_active_collection_id = (
        current_context.get("active_collection_id")
        or ((active_document.get("collection") or {}).get("id") if active_document else None)
    )
    ...
    document_context = {
        ...
        "active_collection_id": resolved_active_collection_id,
        "updated_at": int(time.time()),
    }

    if persist:
        updated_chat = Chats.update_chat_meta_by_id(
            chat.id,
            {
                "document_context": document_context,
                "last_document_sources": ChatMessages.get_latest_sources_by_chat_id(
                    chat.id, db=db
                ),
            },
            db=db,
    ...
    return chat, documents, document_context
...
async def get_chat_documents(
    ...
    chat = _get_chat_for_user(id, user, db)
    chat, documents, _ = _sync_chat_document_context(chat, user, db, persist=False)
    return documents


@router.post("/{id}/documents", response_model=ChatDocumentContextResponse)
async def add_document_to_chat(
    id: str,
    form_data: ChatDocumentAttachForm,
    user=Depends(get_verified_user),
    db: Session = Depends(get_session),
):
    chat = _get_chat_for_user(id, user, db)
    file = Files.get_file_by_id(form_data.document_id, db=db)
    if not file:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=ERROR_MESSAGES.NOT_FOUND,
    ...
    existing_file_ids = {
        chat_file.file_id for chat_file in Chats.get_chat_files_by_chat_id(id, db=db)
    }
    if form_data.document_id not in existing_file_ids:
        Chats.insert_chat_files(
            id,
            form_data.message_id or "document-context",
            [form_data.document_id],
            user.id,
            db=db,
        )

    chat, documents, document_context = _sync_chat_document_context(
        chat,
        user,
        db,
        active_document_id=(
            form_data.document_id if form_data.set_active else None
        ),
    )
    return ChatDocumentContextResponse(
        chat_id=id,
        document_context=document_context,
        documents=documents,
        sources=ChatMessages.get_latest_sources_by_chat_id(id, db=db),
    )


@router.delete("/{id}/documents/{document_id}", response_model=ChatDocumentContextResponse)
async def remove_document_from_chat(
    ...
    chat = _get_chat_for_user(id, user, db)
    Chats.delete_chat_file(id, document_id, db=db)
    chat, documents, document_context = _sync_chat_document_context(chat, user, db)
    return ChatDocumentContextResponse(
        chat_id=id,
        document_context=document_context,
        documents=documents,
...
@router.get("/{id}/context", response_model=ChatDocumentContextResponse)
...
@router.get("/{id}/messages/{message_id}/retrieval-audit", response_model=Optional[dict])
async def get_chat_message_retrieval_audit_by_id(
    id: str,
    message_id: str,
    user=Depends(get_verified_user),
    db: Session = Depends(get_session),
    ...
    return message.get("retrieval_audit")
