...
from open_webui.utils.dochat_documents import (
    ...
    build_document_actor_payload,
    is_document_checked_out_by_another_user,
    is_document_checked_out_by_current_user,
...
def _raise_checkout_conflict(file, user, *, require_current_user_checkout: bool = False):
    ...
    if require_current_user_checkout and not is_document_checked_out_by_current_user(
        file.meta or {}, user.id
    ):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Faça check-out do documento antes de salvar alterações.",
        )


def _ensure_current_user_checkout(file, user, db: Optional[Session] = None):
    if is_document_locked_by_status(file.meta or {}):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=LOCKED_DOCUMENT_DETAIL,
    ...
    if is_document_checked_out_by_another_user(file.meta or {}, user.id):
        _raise_checkout_conflict(file, user)

    if is_document_checked_out_by_current_user(file.meta or {}, user.id):
        return file

    claimed_file = Files.update_file_metadata_by_id(
        file.id,
        apply_document_version_control_metadata(
            file.meta or {},
            user,
    ...
    if not claimed_file:
        raise HTTPException(
            ...
            detail=ERROR_MESSAGES.DEFAULT("Unable to claim document checkout"),
        )

    return claimed_file


def _persist_file_revision_snapshot(
    file,
    content: str,
    user,
    *,
    ...
    version_control = normalize_document_version_control(file.meta or {})
    revision = max(int(version_control.get("revision") or 1), 1)
    created_at = normalize_timestamp(
        (file.meta or {}).get("last_modified_at") or file.updated_at or file.created_at
    ) or int(time.time())

    FileRevisions.upsert_revision(
        file.id,
        revision,
        content or "",
        action=action,
        created_by=build_document_actor_payload(user),
            ...
            FileForm(
                **{
                    ...
                    "meta": {
                        ...
                        "size": len(contents),
                        "data": file_metadata,
                        "uploaded_by": build_document_actor_payload(user),
                        **apply_document_version_control_metadata(
                            {},
                            user,
                            action="upload",
...
@router.post("/{id}/checkout", response_model=Optional[FileModelResponse])
def checkout_file_by_id(
    id: str,
    user=Depends(get_verified_user),
    db: Session = Depends(get_session),
):
    ...
    file = Files.update_file_metadata_by_id(
        id,
        apply_document_version_control_metadata(
            file.meta or {},
            user,
    ...
    if not file:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=ERROR_MESSAGES.DEFAULT("Unable to check out document"),
        )
    return file


@router.post("/{id}/checkin", response_model=Optional[FileModelResponse])
def checkin_file_by_id(
    request: Request,
    id: str,
    ...
    file = _ensure_current_user_checkout(file, user, db=db)

    existing_content = (file.data or {}).get("content", "")
    has_content_changed = form_data.content != existing_content
        ...
        for knowledge in knowledges:
            try:
                ...
                log.warning(
                    ...
                    f"check-in for file {id}: {e}"
                )

    file = Files.update_file_metadata_by_id(
        id,
        apply_document_version_control_metadata(
            (file.meta or {}) if file else {},
            user,
            action="checkin",
            mark_modified=has_content_changed,
            checkin=True,
            bump_revision=has_content_changed,
        ),
        db=db,
    )
    if not file:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=ERROR_MESSAGES.DEFAULT("Unable to check in document"),
        )

    if has_content_changed:
        _persist_file_revision_snapshot(
            file,
            form_data.content,
            user,
            action="checkin",
...
def update_file_data_content_by_id(
    ...
    if (
        ...
        if has_content_changed and file:
            _persist_file_revision_snapshot(
                file,
                form_data.content,
                user,
