<script lang="ts">
	...
	export let className = 'h-full flex pt-4 dochat-messages';
...
<div class={className}>
	...
	{:else}
		<div class="w-full pt-2">
			{#key chatId}
				<section class="dochat-transcript w-full" aria-labelledby="chat-conversation">
...
<style>
	...
	.dochat-transcript ul {
		display: flex;
		flex-direction: column;
		gap: 0.85rem;
	}

	.dochat-transcript :global(.chat-user) {
		color: var(--dochat-text, #1d1d1f);
	}

	.dochat-transcript :global(.user-message .chat-user > div) {
		border: 1px solid rgba(221, 214, 202, 0.82);
		background:
			linear-gradient(180deg, rgba(255, 255, 255, 0.94), rgba(247, 244, 238, 0.92));
		box-shadow: 0 10px 24px rgba(84, 74, 58, 0.04);
	}

	.dochat-transcript :global(.user-message .chat-user .prose) {
		color: var(--dochat-text, #1d1d1f);
	}

	.dochat-transcript :global(.assistant-message-profile-image) {
		box-shadow: 0 10px 24px rgba(84, 74, 58, 0.08);
	}

	.dochat-transcript :global(.chat-assistant) {
		color: var(--dochat-text, #1d1d1f);
	}

	.dochat-transcript :global(#response-content-container) {
		padding: 0.1rem 0 0.15rem;
	}

	.dochat-transcript :global(.buttons) {
		color: var(--dochat-text-soft, #5c5c62);
	}

	.dochat-transcript :global(.buttons button),
	.dochat-transcript :global(.user-message button) {
		color: inherit;
	}
