					...
					>
						...
						>
							{#if items.find((item) => item.model?.connection_type === 'local') || items.find((item) => item.model?.connection_type === 'external') || items.find((item) => item.model?.direct) || tags.length > 0}
								<button
									class="min-w-fit rounded-full px-2 py-0.5 outline-none {selectedTag === '' &&
									selectedConnectionType === ''
										? 'bg-[var(--dochat-accent-soft)] text-[var(--dochat-accent)]'
										: 'text-gray-400 dark:text-gray-600 hover:bg-[var(--dochat-accent-soft)] hover:text-[var(--dochat-accent)]'} transition capitalize"
							...
							{#if items.find((item) => item.model?.connection_type === 'local')}
								<button
									class="min-w-fit rounded-full px-2 py-0.5 outline-none {selectedConnectionType === 'local'
										? 'bg-[var(--dochat-accent-soft)] text-[var(--dochat-accent)]'
										: 'text-gray-400 dark:text-gray-600 hover:bg-[var(--dochat-accent-soft)] hover:text-[var(--dochat-accent)]'} transition capitalize"
							...
							{#if items.find((item) => item.model?.connection_type === 'external')}
								<button
									class="min-w-fit rounded-full px-2 py-0.5 outline-none {selectedConnectionType === 'external'
										? 'bg-[var(--dochat-accent-soft)] text-[var(--dochat-accent)]'
										: 'text-gray-400 dark:text-gray-600 hover:bg-[var(--dochat-accent-soft)] hover:text-[var(--dochat-accent)]'} transition capitalize"
							...
							{#if items.find((item) => item.model?.direct)}
								<button
									class="min-w-fit rounded-full px-2 py-0.5 outline-none {selectedConnectionType === 'direct'
										? 'bg-[var(--dochat-accent-soft)] text-[var(--dochat-accent)]'
										: 'text-gray-400 dark:text-gray-600 hover:bg-[var(--dochat-accent-soft)] hover:text-[var(--dochat-accent)]'} transition capitalize"
						...
						<div class="flex flex-col items-start justify-center py-6 px-4 text-start">
							...
							<a
								href="/admin/settings/connections"
									class="rounded-xl bg-[var(--dochat-accent)] px-4 py-1.5 text-xs font-semibold text-white transition hover:bg-[var(--dochat-accent-hover)]"
								on:click={() => {
									show = false;
								}}
							>
