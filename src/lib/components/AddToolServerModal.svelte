<script lang="ts">
	...
	import { DOCHAT_SITE_URL } from '$lib/constants';
				...
				>
					...
					{#if type === 'mcp'}
						...
						>
							...
							{$i18n.t(
								'MCP support is experimental and its specification changes often, which can lead to incompatibilities. OpenAPI specification support is directly maintained by the Dochat team, making it the more reliable option for compatibility.'
							)}

							<a class="font-medium underline" href={DOCHAT_SITE_URL} target="_blank"
								>{$i18n.t('Read more →')}</a
							>
						</div>
					{/if}
