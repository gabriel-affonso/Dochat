<script>
	...
	import { DOCHAT_SITE_URL, WEBUI_VERSION } from '$lib/constants';
	...
	const saveHandler = async (data) => {
		...
		if (compareVersion(manifest?.required_open_webui_version ?? '0.0.0', WEBUI_VERSION)) {
			...
			toast.error(
				$i18n.t(
					'Dochat version (v{{OPEN_WEBUI_VERSION}}) is lower than required version (v{{REQUIRED_VERSION}})',
					{
	...
	onMount(() => {
		window.addEventListener('message', async (event) => {
			if (![DOCHAT_SITE_URL, 'http://localhost:9999'].includes(event.origin)) return;
